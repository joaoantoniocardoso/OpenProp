...
function [] = Export_SolidWorks_v18_OneLine(filename_SolidWorks,Np,Mp,Z,X3D,Y3D,Z3D)
...
fprintf(fid,'TipSectionCurve%g, ',i);
for j = [1:Np,Np+2:2*Np-1,1] % (2*Np-1 points) does not double print the leading edge
    fprintf(fid,'%f,%f,%f,',X3D(i,j,1),Y3D(i,j,1),Z3D(i,j,1));
end    
...
% Output tip curves
for j = 1:Np-2
    fprintf(fid,'TipCurve%g, ',j);
    i=Mp+1;
    fprintf(fid,'%f,%f,%f, ',X3D(i,   1+j,1),Y3D(i,   1+j,1),Z3D(i,   1+j,1));
    fprintf(fid,'%f,%f,%f, ',X3D(i,2*Np-j,1),Y3D(i,2*Np-j,1),Z3D(i,2*Np-j,1));
end


% Output duplicate root section profile:
i = 1;
fprintf(fid,'RootSectionCurve%g, ',i);
% for each point along the suction and pressure surfaces
